...
Issue reports from the public report form post to `/api/report-issue`, validate against published route/place/stop context, apply a lightweight honeypot/rate-limit guard, and are kept in the mock provider's configured server-side review store. The form also keeps a local browser copy when storage is available. A database-backed report store is still required before switching away from `DATA_SOURCE=mock`.
